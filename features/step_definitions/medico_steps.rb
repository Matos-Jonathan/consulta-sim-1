...
Given('o medico com CRM {string} existe') do |crm|
  fill_in 'medico[nome]', :with => 'Jonathan Weverton'
  fill_in 'medico[cpf]', :with => '706.351.254-47'
  fill_in 'medico[email]', :with => 'jonathan@gmail.com'
...
Given('Clico em editar medico') do
  click_link 'Edit this medico'
  expect(page).to have_content('Editing medico')
end

When('eu atualizo o email para {string}') do |email|
  fill_in 'medico[email]', with: email
end
When('clico em atualizar medico') do
  click_button 'Update Medico'
end
Then('vejo a mensagem que medico foi atualizado com sucesso') do
...
Given('O medico de nome {string}, cpf {string}, email {string}, especialidade {string} e crm {string}') do |nome, cpf, email, especialidade, crm|
  fill_in 'medico[nome]', with: nome
  fill_in 'medico[cpf]', with: cpf
  fill_in 'medico[email]', with: email
  fill_in 'medico[especialidade]', with: especialidade
